...
class FoodController extends ChangeNotifier {
  ...
  void loadFoods({bool approvedOnly = true, String? userId}) {
    ...
    _allFoods = all
        .where((f) => (f.userId == null ? (!approvedOnly || f.isApproved) : f.userId == userId))
        .toList()
      ..sort((a, b) => a.name.compareTo(b.name));
    _applyFilter();
  }
  ...
  void _applyFilter() {
    ...
    if (_selectedSource == 'Universal') {
      list = list.where((f) => !f.id.startsWith('ai_')).toList();
    } else if (_selectedSource == 'Hasil AI') {
      list = list.where((f) => f.id.startsWith('ai_')).toList();
    }
