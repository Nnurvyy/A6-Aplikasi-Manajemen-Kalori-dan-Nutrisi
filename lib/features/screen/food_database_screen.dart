...
class _FoodDatabaseScreenState extends State<FoodDatabaseScreen> {
  ...
  void _loadAndFilter() {
    ...
    if (query.isEmpty) {
      ...
      _dbFoods =
          allFoods.where((f) => f.name.toLowerCase().contains(query)).toList();
    }
    ...
    for (var key in sortedKeys) {
      newGrouped[key] =
          _groupedFoods[key]!..sort((a, b) => a.name.compareTo(b.name));
    }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child:
                _dbFoods.isEmpty ? _buildEmptyState() : _buildCategorizedList(),
          ),
        ],
      ),
    );
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      _nutriChip(
                        'K ${carbs.toStringAsFixed(1)}g',
                        const Color(0xFFFFF8E1),
                        const Color(0xFFF59E0B),
                      ),
                      const SizedBox(width: 4),
                      _nutriChip(
                        'L ${fat.toStringAsFixed(1)}g',
