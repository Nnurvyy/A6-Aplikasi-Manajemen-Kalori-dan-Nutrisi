...
import '../../general/auth/auth_controller.dart'; // ✅ FIX: tambah import
...
class _ScanFoodViewState extends State<ScanFoodView> {
  ...
  void initState() {
    super.initState();
    // ✅ FIX: set userId supaya komposisi manual user terbaca saat scan
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final userId = context.read<AuthController>().currentUser?.id;
      if (userId != null) {
        context.read<ScanController>().setUserId(userId);
      }
    });
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (ctrl.hasResult) ...[
                    ...
                    onPressed: () {
                      ...
                      if (unique.isNotEmpty) {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (_) => ScanResultDetailView(
                                  food: unique.first,
                                  imageFile: ctrl.selectedImage,
                                  initialQuantity: ctrl.getFoodCount(
                                    unique.first.id,
                                  ),
                                ),
                          ),
            ...
            const SizedBox(height: 8),
            Text(
              'Sistem AI akan menghitung kalori makanan secara otomatis',
              textAlign: TextAlign.center,
              style: GoogleFonts.poppins(fontSize: 12, color: Colors.grey),
            ),
            const SizedBox(height: 24),
      ...
      children: [
        Row(
          children: [
            ...
            Text(
              'Hasil Deteksi (${ctrl.mappedFoods.length})',
              style: GoogleFonts.poppins(
        ...
        Row(
          ...
          children: [
            ...
            Text(
              '${ctrl.totalCalories.toStringAsFixed(0)} kkal',
              style: GoogleFonts.poppins(
                fontSize: 18,
                fontWeight: FontWeight.bold,
                color: AppColors.primary,
