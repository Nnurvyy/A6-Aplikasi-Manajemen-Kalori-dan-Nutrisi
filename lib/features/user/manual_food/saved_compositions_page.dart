...
class SavedCompositionsPage extends StatelessWidget {
  ...
  const SavedCompositionsPage({
    super.key,
    this.title = 'Komposisi Tersimpan',
  ...
  Widget build(BuildContext context) {
    ...
    // Hanya tampilkan bahan/makanan manual milik user ini
    final List<FoodModel> savedItems =
        foodCtrl.manualFoods.where((f) => f.isManualIngredient).toList()
      ...
      body:
          displayList.isEmpty
              ...
              : ListView.builder(
                ...
                itemBuilder:
                    (ctx, idx) => _buildCard(context, displayList[idx]),
              ),
    );
  }
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${item.defaultServingSize.round()}g • ${item.nutritionForAmount(item.defaultServingSize)['calories']?.round()} kcal',
                        style: const TextStyle(
                          color: Colors.grey,
                          fontSize: 13,
                      ...
                      Row(
                        children: [
                          _smallNutriChip(
                            'P ${item.nutritionForAmount(item.defaultServingSize)['protein']?.round()}g',
                            Colors.red.shade50,
                            Colors.red.shade700,
                          ),
                          const SizedBox(width: 6),
                          _smallNutriChip(
                            'K ${item.nutritionForAmount(item.defaultServingSize)['carbs']?.round()}g',
                            Colors.amber.shade50,
                            Colors.amber.shade700,
                          ),
                          const SizedBox(width: 6),
                          _smallNutriChip(
                            'L ${item.nutritionForAmount(item.defaultServingSize)['fat']?.round()}g',
                            Colors.blue.shade50,
