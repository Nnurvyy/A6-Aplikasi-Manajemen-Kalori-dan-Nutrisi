...
class _AdminUserManagementViewState extends State<AdminUserManagementView> {
  ...
  Future<void> _deleteUser(UserModel user) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => _ConfirmDialog(
        ...
        message: 'Akun ${user.name} beserta seluruh data log makanannya akan dihapus permanen.',
        confirmLabel: 'Hapus',
                ...
                child: filtered.isEmpty
                    ...
                    : Column(
                        children: [
                          ...
                          if (totalPages > 1)
                            _buildPagination(controller, safePage, totalPages),
                        ],
                      ),
              ),
            ],
  ...
  Widget _buildPagination(AdminUserController controller, int current, int total) {
    return Container(
      color: _surface,
      ...
      child: Row(
        ...
        children: [
          _pageBtn(Icons.first_page_rounded, current > 0, () => controller.setPage(0)),
          const SizedBox(width: 4),
          _pageBtn(Icons.chevron_left_rounded, current > 0, () => controller.setPage(current - 1)),
          ...
          const SizedBox(width: 4),
          _pageBtn(Icons.last_page_rounded, current < total - 1, () => controller.setPage(total - 1)),
        ],
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.person_search_rounded, size: 40, color: _primary),
          ),
          const SizedBox(height: 16),
          Text(
            _searchController.text.isEmpty ? 'Belum ada pengguna' : 'Pengguna tidak ditemukan',
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: user.isBlocked ? const Color(0xFFFFCDD2) : const Color(0xFFE0E0E0),
                    ...
                    children: [
                      ...
                      if (user.isBlocked) ...[
                        ...
                        Container(
                          ...
                          child: const Text(
                            'Diblokir',
                            style: TextStyle(color: _danger, fontSize: 10, fontWeight: FontWeight.w700),
                          ),
                        ),
                      ]
                    ],
...
class _UserDetailSheet extends StatelessWidget {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(user.name, style: const TextStyle(color: _textDark, fontSize: 18, fontWeight: FontWeight.w800)),
                      Text(user.email, style: const TextStyle(color: _textMuted, fontSize: 13)),
                      if (user.isBlocked)
                        Container(
                          margin: const EdgeInsets.only(top: 4),
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                          decoration: BoxDecoration(color: const Color(0xFFFFEBEE), borderRadius: BorderRadius.circular(6)),
                          child: const Text('Akun Diblokir', style: TextStyle(color: _danger, fontSize: 11, fontWeight: FontWeight.w700)),
                        ),
                    ],
                  ),
                ),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _statChip('${user.weight?.toStringAsFixed(1) ?? '-'} kg', 'Berat', const Color(0xFFE8F5E9)),
                      const SizedBox(width: 8),
                      _statChip('${user.height?.toStringAsFixed(0) ?? '-'} cm', 'Tinggi', const Color(0xFFE3F2FD)),
                      const SizedBox(width: 8),
                      _statChip('${user.age ?? '-'} thn', 'Usia', const Color(0xFFFFF8E1)),
                      const SizedBox(width: 8),
                      if (bmi != null) _statChip(bmi.toStringAsFixed(1), 'BMI', const Color(0xFFF3E5F5)),
                    ],
                  ),
                  const SizedBox(height: 14),
                  _sectionTitle('Informasi Akun'),
                  const SizedBox(height: 8),
                  _infoRow(Icons.wc_rounded, 'Jenis kelamin', user.gender ?? '-'),
                  _infoRow(Icons.cake_rounded, 'Tanggal lahir', user.birthDate != null ? dateFormat.format(user.birthDate!) : '-'),
                  _infoRow(Icons.fitness_center_rounded, 'Aktivitas', _shortActivity(user.activityLevel)),
                  const SizedBox(height: 14),
                  _sectionTitle('Target Nutrisi Harian'),
                  const SizedBox(height: 8),
                  _infoRow(Icons.local_fire_department_rounded, 'Kebutuhan kalori', '${user.dailyCalorieNeed?.toStringAsFixed(0) ?? '-'} kkal'),
                  _infoRow(Icons.trending_up_rounded, 'Target BB/bulan', user.targetWeightGainPerMonth != null ? '${user.targetWeightGainPerMonth! >= 0 ? '+' : ''}${user.targetWeightGainPerMonth!.toStringAsFixed(1)} kg' : '-'),
                  _infoRow(Icons.monitor_weight_rounded, 'BB awal', '${user.initialWeight?.toStringAsFixed(1) ?? '-'} kg'),
                  if (user.macroTargets.isNotEmpty) ...[
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          _macroChip('Protein', '${user.macroTargets['protein']?.toStringAsFixed(0)}g', const Color(0xFFEF5350)),
                          const Spacer(),
                          _macroChip('Karbo', '${user.macroTargets['carbs']?.toStringAsFixed(0)}g', const Color(0xFF42A5F5)),
                          const Spacer(),
                          _macroChip('Lemak', '${user.macroTargets['fat']?.toStringAsFixed(0)}g', const Color(0xFFFFA726)),
                        ],
                      ),
                    ),
                  ],
                  ...
                  Row(
                    children: [
                      Expanded(child: _ActionButton(label: 'Edit Data', icon: Icons.edit_rounded, color: Colors.blue, onTap: onEdit)),
                      const SizedBox(width: 8),
                      Expanded(
                        child: _ActionButton(
                      ...
                      const SizedBox(width: 8),
                      Expanded(child: _ActionButton(label: 'Hapus', icon: Icons.delete_rounded, color: _danger, onTap: onDelete)),
                    ],
                  ),
                ],
              ),
  ...
  Widget _sectionTitle(String t) {
    return Text(t, style: const TextStyle(color: _textMuted, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 0.8));
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Text(value, style: const TextStyle(color: _textDark, fontSize: 13, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
  ...
  Widget _macroChip(String label, String value, Color color) {
    return Column(
      children: [
        Container(width: 44, height: 4, decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(2))),
...
class _ConfirmDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Text(message, style: const TextStyle(color: Color(0xFF5A7A5A), fontSize: 14)),
      actions: [
        TextButton(
...
class _EditUserViewState extends State<_EditUserView> {
  ...
  static const List<String> _activities = [
    'Jarang olahraga', 'Sedikit aktif', 'Cukup aktif', 'Sangat aktif', 'Ekstra aktif'
  ];
  ...
  void initState() {
    ...
    _weightCtrl = TextEditingController(text: widget.user.weight?.toStringAsFixed(1) ?? '');
    ...
    _targetCtrl = TextEditingController(text: widget.user.targetWeightGainPerMonth?.toStringAsFixed(1) ?? '0');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Edit Data Pengguna', style: TextStyle(color: _textDark, fontWeight: FontWeight.w800, fontSize: 17)),
        actions: [
          if (isSaving)
            const Padding(
            ...
            TextButton(
              onPressed: _save,
              child: const Text('Simpan', style: TextStyle(color: _primary, fontWeight: FontWeight.w800)),
            ),
        ],
      ),
      body: ListView(
        ...
        children: [
          _buildSection('Informasi Dasar', [
            ...
            _buildField('Email', _emailCtrl, icon: Icons.email_rounded, type: TextInputType.emailAddress),
          ]),
          const SizedBox(height: 12),
          _buildSection('Data Fisik', [
            Row(
              children: [
                Expanded(child: _buildField('Berat (kg)', _weightCtrl, icon: Icons.monitor_weight_rounded, type: TextInputType.number)),
                const SizedBox(width: 10),
                Expanded(child: _buildField('Tinggi (cm)', _heightCtrl, icon: Icons.height_rounded, type: TextInputType.number)),
              ],
            ),
            _buildField('Usia', _ageCtrl, icon: Icons.cake_rounded, type: TextInputType.number),
            _buildDropdown('Jenis Kelamin', _gender, _genders, (v) => setState(() => _gender = v!), icon: Icons.wc_rounded),
          ]),
          const SizedBox(height: 12),
          _buildSection('Target & Aktivitas', [
            _buildDropdown('Level Aktivitas', _activityLevel, _activities, (v) => setState(() => _activityLevel = v!), icon: Icons.fitness_center_rounded),
            _buildField('Target BB/bulan (kg, + naik / - turun)', _targetCtrl, icon: Icons.trending_up_rounded, type: const TextInputType.numberWithOptions(signed: true, decimal: true)),
          ]),
  ...
  Widget _buildSection(String title, List<Widget> children) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: const TextStyle(color: _primary, fontSize: 12, fontWeight: FontWeight.w800, letterSpacing: 0.5)),
          const SizedBox(height: 12),
          ...children
        ],
      ),
